...
export function generateErrorMessage(error: unknown) {
  // 만약 인수로 들어온 error 데이터가 supabase의 auth-js가 제공하는 AuthError 클래스에 있다면.
  // 만약 인수로 들어온 error가 존재한다면.
  if (error instanceof AuthError && error.code) {
    return (
      // AUTH_ERROR_MESSAGE_MAP에서 error.code에 해당하는 키의 값을 찾아 반환.
      // 만약 AUTH_ERROR_MESSAGE_MAP에 데이터가 존재하지 않으면 "알 수 없는 인증 오류가 발생했습니다. 잠시 후 다시 시도해주세요." 문구 반환.
